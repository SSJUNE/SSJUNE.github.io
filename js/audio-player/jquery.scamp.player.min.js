...
}var w=false;var A=0;function E(G){A=0;w=true;}function z(G){w=false;A=0;}function x(G){if(!w){A++;if(A>5){window.removeEventListener("mousemove",x,false);
d.settings.is_mouse=true;}}else{A=0;}}window.addEventListener("mousemove",x,false);window.addEventListener("mousedown",E,false);window.addEventListener("mouseup",z,false);
if(d.settings.mobile){d.settings.autoplay=false;}if(d.settings.mobile&&d.settings.path!=null){function F(){a("a").off("click",F);var G=soundManager.createSound({id:"init_mobile_track",url:d.settings.path+"/ssjune.mp3",autoload:true,volume:100,onload:function(){},onplay:function(){},onpause:function(){},onresume:function(){},onstop:function(){},onfinish:function(){},whileloading:function(){},whileplaying:function(){}});
G.play();}a("a").on("click",F);}if(typeof SC!=="undefined"){SC.initialize({client_id:d.settings.client_id});SC.whenStreamingReady(function(){d.settings.soundcloud=true;
});}d.settings.sm_init=true;var B=null;if(a("> a",d.el).length){B=new Array();var C=a("> a",d.el).length;a("> a",d.el).each(function(){B.push(a(this));
...
var D=sessionStorage.getItem("scamp_player_list"),y=0;a("#sp-queue-scroll tbody").html(D);a("#sp-queue-scroll tbody .sp-track-control").on("click",j);a("#sp-queue-scroll tbody .sp-remove-button").on("click",o);
...
if(sessionStorage.getItem("scamp_player_pos")!==null){y=sessionStorage.getItem("scamp_player_pos");}if(sessionStorage.getItem("scamp_player_shoutcast")==true){g("play",0);
}else{g("play",y);}d.settings.storage_play=true;}else{if(a("#sp-queue-scroll .sp-track.paused").length){d.settings.track=a("#sp-queue-scroll .sp-track.paused");
if(sessionStorage.getItem("scamp_player_pos")!==null){y=sessionStorage.getItem("scamp_player_pos");}g("pause",y);d.settings.storage_play=false;}else{d.settings.track=a("#sp-queue-scroll .sp-track:eq(0)");
...
}}if(sessionStorage.getItem("scamp_player_pos")!==null){y=sessionStorage.getItem("scamp_player_pos");}else{y=0;}g("pause",y);d.settings.storage_play=false;
}}else{if(B!==null){d.settings.obj={status:"working",action:"add_tracks",tracks:B,tracks_size:C,track_nr:-1,};t();}}a(".sp-play-button, .sp-next-button, .sp-prev-button").on("touchstart click",function(){d.settings.play_from_list=false;
});a(".sp-play-button").on("touchstart click",_play);a(".sp-next-button").on("touchstart click",f);a(".sp-prev-button").on("touchstart click",e);a(".sp-queue-button").on("touchstart click",function(G){d.el.toggleClass("sp-show-list");
a("body").toggleClass("sp-show-list");G.preventDefault();});a("#sp-toggle").on("touchstart click",function(G){d.el.toggleClass("sp-show-player");G.preventDefault();
});a("#sp-empty-queue").on("touchstart click",function(G){d.settings.play_from_list=false;if(a("#sp-queue-scroll .sp-track.playing").length||a("#sp-queue-scroll .sp-track.paused").length){_destruct_sound();
...
if(typeof(Storage)!=="undefined"){sessionStorage.removeItem("scamp_player_list");}G.preventDefault();});l();d.update_events("body");d.el.addClass("sp-show");
...
soundManager.setup(d.settings.sm_options);}};_has_touch=function(w){return(("ontouchstart" in window)||(navigator.maxTouchPoints>0)||(navigator.msMaxTouchPoints>0));
};_remove_tracks=function(w){a("#"+w).find(".track").each(function(){var y=a(this).attr("href"),z=a('#sp-queue-scroll .sp-track-control[href="'+y+'"]'),x=a('#sp-queue-scroll .sp-track[data-sc="'+y+'"]');
...
}var x=a("#"+y).find(".sp-track-item .track").length,w=a('#sp-queue-scroll .fromlist[data-fromlist="'+y+'"]').length;if(x!=w){return false;}return true;
};_get_track_index=function(){var w,x=d.settings.track.find(".sp-track-control").attr("href");track_from_queue=a("#sp-queue-scroll").find('.sp-track-control[href="'+x+'"]');
w=track_from_queue.parents('tr[class*="sp-track"]').index();return w;};function v(x,w){if(w==""){w="80";}a.SHOUTcast({host:x,port:w,}).stats(function(){if(this.onAir()){a("#scamp_player .sp-track-container .sp-track-title").text(this.get("songtitle"));
}else{a("#scamp_player .sp-track-container .sp-track-title").text(this.get("status"));}});}var g=function(z,y){if(typeof(y)==="undefined"){y=0;}if(typeof(z)==="undefined"){z="play";
}d.settings.volume_user=d.settings.volume;var x=d.settings.track.find(".sp-track-control").attr("href"),w=document.createElement("a");w.href=x;clearInterval(d.settings.shoutcast_timer);
if(x.indexOf("?sp_radio")>=0){if(typeof(Storage)!=="undefined"){sessionStorage.setItem("scamp_player_shoutcast",true);}a("#scamp_player").addClass("is-shoutcast");
d.settings.is_shoutcast=true;if(d.settings.shoutcast){if(x.indexOf("listen.radionomy")>=0){}else{if(x.indexOf("&onair=0")>=0){}else{d.settings.shoutcast_timer=setInterval(function(){v(w.hostname,w.port);
...
}d.settings.is_shoutcast=false;}d.settings.sound=soundManager.createSound({id:"scamp_sound",url:x,from:y,autoload:false,volume:d.settings.volume,onerror:function(B,A){console.log(this.id+" failed?",B,A);
d.settings.status="error";d.settings.track.addClass("sp-error");d.update_content();if(a("#sp-queue-scroll .sp-track:not(.sp-error)").length){if(d.settings.direction=="prev"){e();
}else{f();}}else{k();}},onload:function(){d.settings.status="play";},onplay:function(){var A={title:d.settings.track.find(".sp-title-button").text(),url:d.settings.track.find(".sp-track-control").attr("href"),artist:d.settings.track.find(".sp-artist-button").text(),track_index:_get_track_index(),tracklist_length:d.settings.tracklist_length};
...
}return true;};var f=function(){d.settings.direction="next";d.settings.storage_play=false;if(d.settings.tracklist_length==null){return;}var y=_get_track_index(),x=y+1,w=null;
if(x>d.settings.tracklist_length){x=0;}_destruct_sound();w=a("#sp-queue-scroll tbody tr:eq("+x+")");d.settings.track=w;_play();if(d.settings.sound){d.settings.sound.setPosition(0);
}return false;};var e=function(){d.settings.direction="prev";d.settings.storage_play=false;if(d.settings.tracklist_length==null){return;}var y=_get_track_index(),x=y-1,w=null;
if(x<0){x=d.settings.tracklist_length;}_destruct_sound();w=a("#sp-queue-scroll tbody tr:eq("+x+")");d.settings.track=w;_play();if(d.settings.sound){d.settings.sound.setPosition(0);
...
}return false;};var l=function(){a(".sp-volume-slider").on("touchstart mousedown",function(D){if(_has_touch()===true&&d.settings.is_mouse==false){var x=true;
}else{var x=false;}if(d.settings.tracklist_length==null){return;}var A=a(this),w=A.find(".sp-volume-position"),C=A.offset().left,z=A.width(),B=d.settings.sound,y=null;
a(document).on("touchmove mousemove",function(G){var F=x?G.originalEvent.changedTouches[0]:G;var E=Math.max(0,Math.min(F.clientX-C,z));y=Math.floor(E/z*100);
w.width(E);if(B){B.setVolume(y);}_set_vol_class(y);return false;}).on("touchend mouseup",function(E){d.settings.volume=y;a(this).unbind("touchmove mousemove");
a(this).unbind("touchend mouseup");return false;}).trigger({type:"touchmove mousemove",clientX:D.clientX});return false;});a(".sp-progress").bind("touchstart mousedown",function(C){if(_has_touch()===true&&d.settings.is_mouse==false){var x=true;
}else{var x=false;}if(d.settings.tracklist_length==null){return;}var y=a(this).addClass("searching"),w=y.find(".sp-postion"),B=y.offset().left,z=y.width(),A=d.settings.sound;
d.settings.searching=true;if(x==false){if(A){xPos=Math.max(0,Math.min(C.pageX-B,z));w.width(xPos);A.setPosition(xPos/z*A.durationEstimate);}}A.pause();
a(document).bind("touchmove mousemove",function(F){var E=x?F.originalEvent.changedTouches[0]:F;var D=Math.max(0,Math.min(E.clientX-B,z));w.width(D);if(A){A.setPosition(D/z*A.durationEstimate);
}return false;}).bind("touchend mouseup",function(D){d.settings.searching=false;y.removeClass("searching");if(d.el.hasClass("playing")&&A){A.resume();}a(this).unbind("touchmove mousemove");
a(this).unbind("touchend mouseup");return false;}).trigger({type:"touchmove mousemove",clientX:C.clientX});return false;});};_set_vol_class=function(w){if(w==0){a(".sp-volume-button").removeClass("sp-vol-medium sp-vol-max").addClass("sp-vol-off");
...
y.stopImmediatePropagation();return false;};_play_track=function(A){if("#"==a(this).attr("href")||""==a(this).attr("href")){return false;}if(d.settings.obj.status=="working"){return false;
}if(a(this).parents(".sp-list")){var C=a(this).parents(".sp-list");if(C.attr("id")){if(!_check_tracks(C.attr("id"))){var y,z=a(this).parents(".sp-track-item");
a(".sp-track-item",C).each(function(D){if(z.is(a(this))){y=D;return false;}});_remove_tracks(C.attr("id"));_create_tracks(C.attr("id"),"play_tracks",y);
return false;}d.settings.play_from_list=true;}}var w=_pretty_link(a(this)),x=[w];if(d.settings.mobile){var B="add_track";}else{var B="play_track";}d.settings.obj={status:"working",action:"play_track",tracks:x,tracks_size:1,track_nr:-1};
t();A.preventDefault();A.stopPropagation();A.stopImmediatePropagation();return false;};_add_tracks=function(w){var x=a(this).data("id");if(x==undefined||x==""||a("#"+x).length<=0){return false;
...
}};var n=function(w){a(w).bind("touchstart mousedown",function(D){if(_has_touch()===true&&d.settings.is_mouse==false){var y=true;}else{var y=false;}if(d.settings.tracklist_length==null){return;
}var z=a(this).addClass("searching"),x=z.find(".sp-content-postion"),C=z.offset().left,A=z.width(),B=d.settings.sound;d.settings.searching=true;B.pause();
if(y==false){if(B){xPos=Math.max(0,Math.min(D.pageX-C,A));x.width(xPos);B.setPosition(xPos/A*B.durationEstimate);}}a(document).bind("touchmove mousemove",function(G){var F=y?G.originalEvent.changedTouches[0]:G;
var E=Math.max(0,Math.min(F.clientX-C,A));x.width(E);if(B){B.setPosition(E/A*B.durationEstimate);}return false;}).bind("touchend mouseup",function(E){d.settings.searching=false;
z.removeClass("searching");if(d.el.hasClass("playing")&&B){B.resume();}a(this).unbind("touchmove mousemove");a(this).unbind("touchend mouseup");return false;
}).trigger({type:"touchmove mousemove",clientX:D.clientX});return false;});};d.update_events=function(w){a(w).off("click",".sp-add-track",_add_track);a(w).off("click",".sp-play-track",_play_track);
...
case"error":B.addClass("sp-error");break;case"stop":B.addClass("sp-stop");a(".sp-content-control.sp-is-active").removeClass("sp-is-active");break;}}});
};d.addTrack=function(w,y){if(d.settings.obj.status=="working"){return false;}if(typeof w=="undefined"){return false;}var w=a('<a href="'+w.url+'" data-cover="'+w.cover+'" data-artist="'+w.artist+'" data-artist_url="'+w.artist_url+'" data-artist_target="'+w.artist_target+'" data-release_url="'+w.release_url+'" data-release_target="'+w.release_target+'" data-shop_url="'+w.shop_url+'" data-shop_target="'+w.shop_target+'" data-free_download="'+x[i].free_download+'">'+w.title+"</a>"),x=[w];
